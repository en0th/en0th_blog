...
```
在安装过程中我需要了下面的报错问题。
![image.png](../../images/posts/3823075d57a2e0b0288817983a326702.png)
解决命令如下：
```
yarn remove webpack
...
```
![image.png](../../images/posts/7941a0565af4a7cb2b9c0b7797ebd156.png)
后台查看SQL执行情况：
![image.png](../../images/posts/0661f0b4b83ce853c05560a069b110e7.png)
预编译的SQL语句为：
```
...
![image.png](../../images/posts/258327f45606a799fdf5b9f756c4cb12.png)
```
...
```
正确后正常返回数据。
![image.png](../../images/posts/a8fba323dbb8cc5f43771dd1b3ff6f6e.png)
#### 【后台】漏洞位置：计量单位
访问 [http://localhost:10003/goodsUnit](http://localhost:10003/goodsUnit) 登录后台并进入到`商品->计量单位`界面。![image.png](../../images/posts/662d9a793f3a0b7b0ccbafc93c80e67f.png)
...
```
![image.png](../../images/posts/bdf161d70cc0a5f7e739403d25162c45.png)
后台查看SQL执行情况：
![image.png](../../images/posts/0ddda5faba4e5157d70cddd4ce8e155d.png)
预编译的SQL语句为：
```
...
拦截功能配置：[配置 wallfilter](https://github.com/alibaba/druid/wiki/%E9%85%8D%E7%BD%AE-wallfilter)
 默认函数黑名单：
![image.png](../../images/posts/60243be202c3ca5df86541cda296a944.png)
### 漏洞分析
 其中的`initPage`函数处理中使用了`addOrder`但是没有对`sort`进行SQL语句过滤。
`framework/src/main/java/cn/lili/mybatis/util/PageUtil.java`
![image.png](../../images/posts/2828339a3270ce7772a521f92253a5c6.png)
全局搜索`PageUtil.initPage`
![image.png](../../images/posts/cc1d23c6faac33f50563754d4d50aeec.png)
...
![image.png](../../images/posts/76d5bc8be4f386872848d6ae6952ce07.png)
断点测试时，发现存在恶意的`sort`内容被保留，并在后续拼接到预编译SQL语句。
![image.png](../../images/posts/7e78fac8f30d003a70044d81e828bcee.png)
...
![image.png](../../images/posts/365ddcc39fbeb327fd88f5cf1779cf14.png)
我选择使用 payload
```python
...
![image.png](../../images/posts/6808531dd5048a6f684fc6668ba69674.png)
准备好这个之后，我们有很多出地方可以出发这个请求。
比如这里是商家查看获取物流，我们还可以在买家端、运营端找到这个功能。
这里是需要存在一个订单处于已发货状态。
![image.png](../../images/posts/22650be1cf1f7b67acdee21c819a295c.png)
当我们点击后在`dnslog.cn`可以获得解析记录。
![image.png](../../images/posts/b0dc42886d6533b707f7ef4ee6881756.png)
Fastjson <= 1.2.80 可以打三种不同的利用链，这里我发现存在其中一种利用链`groovy`。
`org.codehaus.groovy.control.CompilationFailedException`
![image.png](../../images/posts/44afc776166959e996d5de72451f9441.png)
改写我们的Exp python文件。
```python
...
...
![image.png](../../images/posts/3ae346f98c95e7e2aaa2b19e037acdbe.png)
通过`maven package`打包成 jar，然后到jar的目录下执行`python -m http.server 35260`
接下来分别让FastJson解析两个payload后就执行了我们想要的命令。
![37.png](../../images/posts/85313bd2dc9bdc78c354b6060b9c9a37.png)
总的过程：
...
![过程.gif](../../images/posts/c9d2c9c2da0715f7b20e8b488c32a457.gif)
> 参考资料：
...
![image.png](../../images/posts/533bcc7ed194a6eb469e733a61445adb.png)
保存后，我们登录商家端，在`订单->商品订单`找一个待发货的订单，进入订单详情后，点击`打印电子面单`然后点击确认。
![image.png](../../images/posts/d3c9828ea23434d32b28a9bd6613fda2.png)
脚本可以收到请求信息
![image.png](../../images/posts/accb42e17b2c3d2a982f52f475250eac.png)
按照上面发送两个请求后一样能够RCE。
![image.png](../../images/posts/fe9a215587baea9658095f7f04913f37.png)
### 漏洞分析
#### 查看物流处利用
...
![image.png](../../images/posts/8de51d34df68096031baeb7d2b1c5d92.png)
这里的`ReqURL`是由快递鸟设置的。
![image.png](../../images/posts/16791ce1056dcb2932ed1fcf8599f961.png)
只有同一文件中的函数`getLogistic`调用了它。
![image.png](../../images/posts/acdde4a8f39ed1633f5affbad7c6970a.png)
但它存在两个用法，分别是`AfterSaleServiceImpl`和`OrderServiceImpl`，分别是查看订单里的查看物流功能和退货后里的查看物流功能，总共有五处可以触发该功能。
![image.png](../../images/posts/0fe44b6449925c3944d9fe9c8f8e5052.png)
查看物流实现的是`getTraces`接口。
`framework/src/main/java/cn/lili/modules/order/order/serviceimpl/OrderServiceImpl.java`
![image.png](../../images/posts/20b5eb3571665f47acd0f75b49a3be20.png)
有三处调用，分别是买家、运营、商家。
![image.png](../../images/posts/7c2fbaf803aa8025057e478daaae2775.png)
#### 打印电子面单处利用
`framework/src/main/java/cn/lili/modules/kdBrid/serviceImpl/KdNiaoServiceImpl.java`
![image.png](../../images/posts/76ac95be41847b33959291ff48cdc82d.png)
![image.png](../../images/posts/a6e05e5f3fd2f6832b1c87e21dd378d6.png)
`seller-api/src/main/java/cn/lili/controller/order/OrderStoreController.java`
![image.png](../../images/posts/c8a8c1db2cb2373641f5d40507fd9765.png)
...
`设置->信任登录`填写好appid和appSecret。![image.png](../../images/posts/4d3e64afc7045bb4bd6afe0e6074e80f.png)
然后进入卖家后台[http://localhost:10002/liveGoods](http://localhost:10002/liveGoods)
`营销->直播商品`选择添加商品，任意选择商品点击确定。
![image.png](../../images/posts/e3d0da70a6fb67cf6242b9fecd46c1bd.png)
抓包后修改`goodsImage`参数即可。
![image.png](../../images/posts/e7811a9a92e50d2f94afb8a0652a7d1e.png)
监听的服务能够接收到请求。
![image.png](../../images/posts/ff294e677d296c6077c105387c7a9213.png)
请求数据包：
```
...
![image.png](../../images/posts/1bfcc5ef9648964169ad53b1eec2bd5b.png)
