...
# 0x01 声明
**<font style="color:rgb(36, 41, 47);">遵纪守法</font>**  
<font style="color:rgb(36, 41, 47);">公网上存在部署了旧版本的CMS，旧版本仍然存在这些问题。</font>  
<font style="color:rgb(36, 41, 47);">请不要非法攻击别人的服务器，如果你是服务器主人请升级到最新版本。</font>  
<font style="color:rgb(36, 41, 47);">请严格遵守网络安全法相关条例！此分享主要用于交流学习，请勿用于非法用途，一切后果自付。</font>  
<font style="color:rgb(36, 41, 47);">一切未经授权的网络攻击均为违法行为，互联网非法外之地。</font>  
**<font style="color:rgb(36, 41, 47);">漏洞报送</font>**  
<font style="color:rgb(36, 41, 47);">该文章涉及的漏洞已提交到CNVD、CNNVD平台。</font>  
**<font style="color:rgb(36, 41, 47);">文章转载</font>**  
<font style="color:rgb(36, 41, 47);">商业转载请联系作者获得授权，非商业转载请注明出处。</font>  
<font style="color:rgb(36, 41, 47);">作者公众号：响尾蛇社区</font>
...
## 【高危】前台任意文件上传漏洞
### 漏洞详情
在实现上传头像功能上，存在远程拉取图片的方法，使用了`<font style="color:#080808;">CurlUtil::getRaw</font>`<font style="color:#080808;">远程获取文件内容并通过</font>`<font style="color:#080808;">file_put_contents</font>`<font style="color:#080808;">保存该文件到</font>`<font style="color:#080808;">temp</font>`<font style="color:#080808;">目录下。攻击者可以构造恶意的URL，利用这个功能来上传恶意文件。</font>
...
![](../../images/posts/04ba403420b895a38c974c49843754e6.png)

访问 http://localhost:38201/register 任意注册账号。

![](../../images/posts/2432d568c492572882910623964c8d37.png)

访问 http://localhost:38201/login 进行登录，登录成功后找到修改头像功能点。

http://localhost:38201/member_profile/avatar

![](../../images/posts/b7feeb501215e8ab1ccd339a373cfc88.png)

任意上传一张图片并进行抓包修改。

原始数据包：

![](../../images/posts/87879c77a26d4fbaba213994e71eedd1.png)

修改后：

![](../../images/posts/63e7cee4bb400fdf65bdf2eecff536ca.png)
...
<font style="color:#080808;background-color:#ffffff;">生成的文件名是md5值，摘取内容为：</font>

```plain
yMZzWpFRGBGwrBVwDiNQF0U9xl7Baqa8:http://127.0.0.1:19980/help.PHP
```

<font style="color:#080808;background-color:#ffffff;">将内容md5编码后得到</font>`ce540a9b00ee6fefa1f0ead15f2895ba`

![](../../images/posts/0cde45e95875ebd56c3b7e0395cc323d.png)

访问 http://localhost:38201/temp/ce540a9b00ee6fefa1f0ead15f2895ba.php

![](../../images/posts/574ea80eae832e02583d5207c2498bb8.png)
...
![](../../images/posts/98717c30e7a611e424df3dea1f66f0f8.png)

跟进后来到`Member/Api/Controller/MemberProfileController.php`的`avatar`函数中进行处理。通过改变传入`<font style="color:#080808;">type</font>`<font style="color:#080808;">参数的值来进入到</font>`<font style="color:#080808;">savePathToLocalTemp</font>`<font style="color:#080808;">函数中去。</font>

![](../../images/posts/e710a793339627aae0d4754a5c807e6c.png)

`<font style="color:#080808;">savePathToLocalTemp</font>`<font style="color:#080808;">函数中通过</font>`<font style="color:#080808;">CurlUtil::getRaw</font>`<font style="color:#080808;">请求了我们的链接并通过</font>`<font style="color:#080808;">file_put_contents</font>`<font style="color:#080808;">保存文件到</font>`<font style="color:#080808;">$tempPath</font>`<font style="color:#080808;">指定的目录。这种拼接方式是可以被预测的  ，</font>`<font style="color:#080808;">$appKey</font>`<font style="color:#080808;">默认为</font>`<font style="color:#080808;">yMZzWpFRGBGwrBVwDiNQF0U9xl7Baqa8</font>`<font style="color:#080808;">。</font>

```plain
public_path('temp/' . md5($appKey . ':' . $path) . (starts_with($ext, '.') ? $ext : '.' . $ext));
```

![](../../images/posts/03ec686b0e42c2088ea7958f95437016.png)
...
## 【高危】前台远程命令执行（RCE）漏洞
### 漏洞详情
在实现上传头像功能时，不安全的使用了`<font style="color:#080808;">file_exists</font>`<font style="color:#080808;">函数，将未经校验的数据传递给了该函数。攻击者可以构造恶意的phar文件，使用phar协议触发反序列化，造成远程执行漏洞。</font>
...
1. 注册账号并登录
2. 上传带有反序列化利用的phar文件
3. 通过 <font style="color:#080808;">file_exists 函数触发phar协议</font>
1. **<font style="color:#080808;">注册账号并登录</font>**
...
2. **<font style="color:#080808;">注册账号并登录</font>**
...
3. **<font style="color:#080808;">生成EXP phar文件</font>**
...
4. **<font style="color:#080808;">上传phar文件</font>**

在**相同游览器**访问`http://localhost:19980/`页面，选择并上传`laravel5_exp.jpg`文件。

![](../../images/posts/a20dbe0cc2eb303eda8d57e30a7f9808.png)

上传后得到路径`\/data\/file\/2023\/09\/22\/28396_a9tq_5362.jpg`

![](../../images/posts/ed5e58c7b70e9c9b99af4a5d4a3d76d3.png)

5. **<font style="color:#080808;">触发phar协议造成反序列化</font>**
...
![](../../images/posts/790f30c7eb10e9766556dfc154daa35d.png)
...
![](../../images/posts/4ca39b9fbd4410b36c4561d66e01d540.png)

[此处为语雀卡片，点击链接查看](https://www.yuque.com/docs/167865420#oOGVo)
...
传入的路径会先通过`<font style="color:#080808;">file_exists</font>`<font style="color:#080808;">函数进行判断，这个函数可以触发phar协议，执行反序列化，造成命令执行。</font>

![](../../images/posts/416ef63278550b39a41f8cb97aba2f8d.png)
...
![](../../images/posts/d5bcc6436ce67c7fc88bac2cb0117e4b.png)

具体处理函数`php_unserialize_with_options`可以在`php-src/ext/standard/var.c`中找到。

![](../../images/posts/60d556c03863a5348c60cc552266e8b0.png)
...
从下面代码看，尽管我们不传递`options`保持为NULL一样可以进入到`<font style="color:rgb(15, 15, 15);">php_var_unserialize</font>`<font style="color:rgb(15, 15, 15);">函数中去。这种代码属于向后兼容性。</font>

![](../../images/posts/b6b5e7abcfcbd09accd78c596f7efdba.png)

在`php-src/ext/phar/phar_object.c`我们可以了解到，`Phar::getMetadata`C 扩展宏定义。

![](../../images/posts/18de97fbead2e5f7600704fa54ab1abb.png)
...
![](../../images/posts/c291e10e6aa31901244d5b53a2ff6ff6.png)
