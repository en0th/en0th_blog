...
> <font style="color:rgb(18, 18, 18);">.Net Framework 和 .Net Core 都包含了</font>ASP.net<font style="color:rgb(18, 18, 18);">，但是.Net Core中的</font>ASP.net<font style="color:rgb(18, 18, 18);">被重新设计过了，目前没有看到Web Form这个功能，只看到了MVC这个功能。</font>
>
...
<font style="color:rgb(36, 41, 47);">请严格遵守网络安全法相关条例！此分享主要用于交流学习，请勿用于非法用途，一切后果自付。  
</font><font style="color:rgb(36, 41, 47);">一切未经授权的网络攻击均为违法行为，互联网非法外之地。</font>
...
![](../../images/posts/45e076caeb60e5e6008a1b001a9c1bd6.png)
...
![](../../images/posts/d76eed6a359e1c7b877020761b7331a9.png)

修改完后右键数据库选择恢复数据库。

![](../../images/posts/67bc6d24df1c0c02b272cbe25dc65a59.png)

通过这个功能分别导入两个sql文件。

![](../../images/posts/4a5eea3a4f2f0913f7301ab672ed0327.png)

修改配置文件应用Mysql服务。

`BootstrapAdmin\src\mvc\admin\Bootstrap.Admin\appsettings.json`

![](../../images/posts/d93055d82ff636497c351ac84624a863.png)

`BootstrapAdmin\src\mvc\admin\Bootstrap.Admin\appsettings.Development.json`

![](../../images/posts/6b0ac41873bd07032bfa5e91b46b9d9c.png)

重新生成后出现以下错误。

![](../../images/posts/44d5614461d768c53361c0e804d78435.png)

在Visual Studio帮助旁边的搜索栏搜索 Nuget

![](../../images/posts/abf8179b6d5a32bda805f36a5f7b6263.png)

在弹出的窗口选择`游览`，搜索Mysql，下载安装`Mysql.Data 8.029`这个版本。

因为最新版本不支持.Net5.0。

![](../../images/posts/956d8f945ae59d76d902498b8a00fa5b.png)
...
![](../../images/posts/21039baa0ba24ad73523f63c33a59b74.png)
...
![](../../images/posts/147b358f70ca9156d653bf1fe96ceceb.png)

当我们点击返回首页时执行了我们的跨站脚本，可以在xssye.com后台中看到数据，但是没有获取到Cookie。

![](../../images/posts/e318936abf437982010fb0e09c7d5c66.png)

Cookie都有 HttpOnly 所以获取不到。

![](../../images/posts/09612eb05f031dc5613e153b83191f0a.png)

### 漏洞定位
`BootstrapAdmin\src\mvc\admin\Bootstrap.Admin\Controllers\HomeController.cs`

![](../../images/posts/e7583de3733b01f35a1875619ba6a337.png)
...
![](../../images/posts/1da4ee76a15ef710708ee9b7902fe308.png)
...
![](../../images/posts/2e4ae4badc5b3294efca5ddd98b98688.png)
...
![](../../images/posts/36d0bf9219120fe55ea8d416db714fec.png)
...
![](../../images/posts/c945cbcb647c413ae5efb96d4e2491fc.png)
...
![](../../images/posts/49c635d1248633f0db0e8cfca88bc452.png)

通过使用 payload`/../../../../../../../../delete_me.txt`将文件删除了。

![](../../images/posts/e1458e53350a76fedada77bd91827c05.png)
...
![](../../images/posts/c5d9cbc3122004a62a86bcac1258f6e9.png)
...
![](../../images/posts/6929491fb22899fdd4cae27311a3b8c1.png)
...
![](../../images/posts/d3efe954501d1a9a01e5878733080d25.png)

我们退出Admin账户，重新登录root账户，然后到个人中心处上传图片后抓包。

![](../../images/posts/d4705dda851932ad7313d3297d0de2cf.png)
...
![](../../images/posts/5ac175e6ee0e70dc9d6a5ab94205528a.png)

解码之后发送请求。

![](../../images/posts/ee5c42c7c91e1789c9b5b1002da3b4b5.png)

到测试服务器上查看，发现已经写入。

![](../../images/posts/3794aa481d3a5142d716d9b58742fbc0.png)

Windows 开 nc 监听等待一分钟也能连上，至此成功Getshell。

![](../../images/posts/47ad5409abba24e52d23189f4d34fe11.png)
...
![](../../images/posts/36e3d5abfd9a89c37a1a8cbd8e9f6c8b.png)
...
![](../../images/posts/51d8d9118ddf50cc094a0e99ba6c228b.png)

设置了这个中间件去访问动态文件 asp 时会因为`app.UseStatusCodePagesWithReExecute("/Home/Error/{0}");`返回`/Home/Error/404`的界面。

访问：`http://localhost:50852/Admin/Profiles`

![](../../images/posts/ef845e1d9a8d5e1fe8685857e0e01d41.png)

找到修改头像，选择一句话木马后上传，抓包修改后缀名为 asp 放包即可。

![](../../images/posts/748a4c8fb0ea12acda037a09b94e0739.png)

可以看到文件已经上传成功了。

![](../../images/posts/e955b9a087cc825b56cab27cbc6691a2.png)

虽然文件上传成功，但很可惜，无法解析。

![](../../images/posts/25cae99a62a25ba6e3c40d25ab99ba44.png)

我注意到`fileName = $"{userName}{Path.GetExtension(uploadFile.FileName)}";`

路径拼接中使用了`userName`，那我可以尝试通过修改用户名来达到目录穿越的目的。

更新用户名，`PUT http://localhost:50852/api/Profiles`![](../../images/posts/ee33d5fccec19515542afcacc55583c0.png)

很可惜存在 UserName与 当前登录用户名进行判断，我们没办法通过这个判断。

`BootstrapAdmin\src\mvc\admin\Bootstrap.Admin\Controllers\Api\ProfilesController.cs`

![](../../images/posts/bde289dc4b10a33a042f209726a72f66.png)

还有一个地方可以编辑，那就是用户管理。

![](../../images/posts/494a85efc53998bc598c70568acb296e.png)

抓包修改之后修改UserName，但是实际上没有修改成功。

![](../../images/posts/c024bc662f985ce4020a4ec4031c1f2e.png)

这里没有使用到我们的 UserName，但我们可以新建一个账户。

`BootstrapAdmin\src\mvc\admin\Bootstrap.Admin\Controllers\Api\UsersController.cs`

![](../../images/posts/f8a629eb2e73b26049487bf7be731d28.png)

随意创建一个用户。

![](../../images/posts/0042a225441d3695a1976df1fae000f6.png)

修改请求中的 UserName

![](../../images/posts/8eb52c9774f0c9a97656d824eae76116.png)

`BootstrapAdmin\src\mvc\admin\Bootstrap.Admin\Controllers\Api\UsersController.cs`

![](../../images/posts/35ad61a9852f66711a83bc8434e31cfe.png)

进入到 UserHelper.Save

`BootstrapAdmin\src\mvc\admin\Bootstrap.DataAccess\Helper\UserHelper.cs`

![](../../images/posts/349ba411e29797f65bbc1be82440e6b4.png)

继续进入到 UserHelper.UserChecker，其中针对我们传入的 UserName 进行了长度限制和正则匹配。很显然我们输入的`..\\..\\..\\..\\`没法通过匹配。

![](../../images/posts/835020a8a9817ce1871b343d345401df.png)
...
![](../../images/posts/f7a9caef2fea2a0bef2a4720e0ce53e0.png)

将字典代码内容修改成`~/../../../../../../../`。

![](../../images/posts/f4ae464a7b1b142f09d342b118ae20f7.png)

这个时候我们再次上传就可以看到路径已经拼接好了。

![](../../images/posts/47118372b614db42c0913968aa0f3e9b.png)

![](../../images/posts/ba6fb9b8e42aecc58853a557561a1c4c.png)
...
![](../../images/posts/fb967212932847aa3f1f17ad171e56bf.png)

所以我想到需要编辑Hex，而BurpSuite2020及之后版本都没法直观的编辑Hex。

官方给的说明如下（Google翻译过后的）

![](../../images/posts/658ec1fa6df3b7d4d1bc242efc18b449.png)
...
![](../../images/posts/d94778501f886205dbd2041acf096b10.png)
...
![](../../images/posts/242f682c2f5226d9609bfb66e9251f99.png)

点击申请账号，任意填写内容后点击提交并抓包。

![](../../images/posts/1bb988d1b0ab5c23985397fc32705838.png)
...
![](../../images/posts/82124cdd9997bcbe2e36cd01e335c654.png)
...
![](../../images/posts/7c0eb052e999695f7934db642f11bfe9.png)

使用我们刚刚注册的账号进行登录。可以看到能够登录，也就是说**绕过了注册账号需要管理员通过的操作**

![](../../images/posts/d62deea43ef6c53670f08195657d040e.png)

可以看到是 test 账户，现在是默认权限的状态。

![](../../images/posts/447f4e8745a79506479210ffc80c029c.png)
...
![](../../images/posts/58656f251979c31d80f1248c15b3d82f.png)

![](../../images/posts/eb7eaa8584b67804b8a337923430351c.png)
...
![](../../images/posts/4bffea3da04a87304c80c5612dd7591d.png)
...
![](../../images/posts/75a9753006dab287a898379d2a205cea.png)

`BootstrapAdmin\src\mvc\admin\Bootstrap.DataAccess\User.cs`

到这里直接通过`db.Insert`操作将我们传入的所有数据进行了保存操作。

![](../../images/posts/17183588509c5e15ba48e48ec448fe2c.png)
...
![](../../images/posts/221fc3d6b9b48be6f0b94ff2bd8e31b2.png)

只要用户名和密码不为空就进入到 `UserHelper.Authenticate`。

`BootstrapAdmin\src\mvc\admin\Bootstrap.DataAccess\Helper\UserHelper.cs`

![](../../images/posts/87b40cc1a1553372498d9b80e6c4dca1.png)

这里进入到`Authenticate`函数

`BootstrapAdmin\src\mvc\admin\Bootstrap.DataAccess\User.cs`

![](../../images/posts/fc9119c50cd589729ee5ef5c55107e55.png)
...
![](../../images/posts/27120acae6a25c5a1787f44843bc0124.png)

进入到忘记密码界面，账号处输入`Admin`即默认管理员账户登录名称，其他字段信息随意填写。

![](../../images/posts/1126783b7f33e02d749acc44787bfdec.png)

我们提交之后再发送一个重置密码的包即可，这个包不需要任何权限。

![](../../images/posts/065c7a9950c3d638bca90dc0fbf85fc2.png)
...
![](../../images/posts/a3b6d3c914c20381bb6cc5088782bf2c.png)
...
![](../../images/posts/6fe2a0b5be2e2d371083614636f9c8bb.png)
...
![](../../images/posts/feb8ffcd4db725cbd78c6ac1c34dcc31.png)
...
![](../../images/posts/0f2f6ab0da58c97c4c744cf2adfbcd41.png)
...
![](../../images/posts/b25f97206aa7347abd324de90b646b30.png)
...
![](../../images/posts/ae16c35af1af63eb834fc9629bb96b5e.png)
...
![](../../images/posts/62dd5cd7c78be6a68e56d638070fae73.png)
...
![](../../images/posts/24863a36b5e9de3202a54f6b02f2de44.png)
...
![](../../images/posts/127b5816031575d08d07627813759e5d.png)
...
![](../../images/posts/061da962ab0271b35ee9562f66c26e84.png)
...
![](../../images/posts/be44f4a2270af6c182e59ae13af31845.png)
...
![](../../images/posts/847f00d43d222f8b1a5d34ed277ac9f6.png)
...
![](../../images/posts/81a75b392674e41d44c192aed653bde6.png)
...
![](../../images/posts/4cf630f90aef7e3d98b1dd677c4f6715.png)
...
![](../../images/posts/b33eb6a1127f0f9c112127f87e4f609c.png)
...
![](../../images/posts/8bd0cf87715398a7cf0ba16e8a0317c9.png)
...
![](../../images/posts/7352306b31d4773433705f0262964b64.png)
...
![](../../images/posts/48b6d7a4bebe2abb559125b4eab817ce.png)
...
![](../../images/posts/548a115dee49b7b3bc5947430b8dada0.png)
...
![](../../images/posts/01d2a5098a0eefcd05d7563297f3112a.png)
...
![](../../images/posts/60bb9e1527c8ee018cae3e134e45fa77.png)
...
![](../../images/posts/4523927e70541fb7c0e52ce7c2005962.png)
...
![](../../images/posts/500103945c347afe15d888827cba57e2.png)
...
![](../../images/posts/a03fbd933ceeddf6c0b266471ab75f23.png)
...
![](../../images/posts/10c3d3fcfdd413cba10ebd60ce96f24c.png)
...
![](../../images/posts/c57037cdfee20fb69a35d457220d219b.png)

然后给这个账户增加管理员权限，同样使用JWT验证。

![](../../images/posts/b3b4e34f2cd40fb059d4b4c807a77239.png)
...
![](../../images/posts/e4c7e8cc66f88571872b8368620df26d.png)
...
![](../../images/posts/98a89d0c1591e9bd2f5444c26a9f91cb.png)
...
![](../../images/posts/3422ef6b59d9d8e1a15c27498536ff40.png)
...
![](../../images/posts/f4c80cf9ba8be6cf7c6c4c6fc9ddb524.png)

需要校验的内容都在：

`BootstrapAdmin\src\mvc\admin\Bootstrap.Admin\appsettings.json`

![](../../images/posts/4f2cb4198e37b425575c790e2aa3d67a.png)
...
![](../../images/posts/aff73331de6304d2338fed481429f0cf.png)
...
![](../../images/posts/e1e50ab7f1fe88daa2e5e3464754a72d.png)

反编译`bootstrap.security.mvc\6.0.0\lib\net5.0\Bootstrap.Security.Mvc.dll`

跟进 `BootstrapAdminAuthorizeFilter`类可以看到`OnAuthorizationAsync`，这方个法适用于控制器和 Razor 页面等需要进行授权检查的请求。

![](../../images/posts/04ed0410f79227bdf6d9940bc1463b75.png)
