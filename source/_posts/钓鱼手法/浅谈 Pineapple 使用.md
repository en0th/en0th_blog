...
![](../../images/posts/df38fd5afa9dbd65a4b68eb3236d4d87.png)
...
![](../../images/posts/82cfb389c2af490b83fb4735c302bf5a.png)
...
![](../../images/posts/b927ae644f827ecf12bf80a154ab6bb2.png)

修改后访问后台系统地址：`http://172.16.42.1:1471/#!/modules/Setup`

![](../../images/posts/b329c472d92937f245caebf42911017c.png)

看到下面这个提示，拿小针戳一下重置的小按钮，快速戳一下就放开就可以进入到Setup界面了。

![](../../images/posts/2332e1dad23b6c878e3d237f71bc4539.png)

进入到设置页面，先进行密码设置。这个密码即是后台系统访问密码，也是SSH访问密码。机器默认开放`22、1471、53、80`端口。

![](../../images/posts/f98e2c7913eccc63b2e6db1c281140e2.png)

在这里设置你的管理AP和开放AP。你可以选择隐藏它们。接入它们都可以访问后台。

![](../../images/posts/466ce24e7e39e741a3ac1ec2ad65807e.png)

过滤器用于选择禁止某些设备连接WIFI。**这里需要都选择黑名单的方式。否则后续无法连接WIFI。**

![](../../images/posts/649cbaadb0af714f1de19815a5f8751d.png)
...
![](../../images/posts/f4480510a9103af936fd9d40247ec729.png)

填入密码后点击连接。

![](../../images/posts/6b4e43dc5f8e66bbfdcef7743548577d.png)
...
![](../../images/posts/42ddd85655c367aefb534691e8515560.png)

也可以使用ssh 登录，ip 为 172.16.42.1 ，端口号为 22，用户名为 root，密码为之前设置的密码。

![](../../images/posts/4569b9098191b8042121bbbd9c551194.png)
...
![](../../images/posts/30ac44f25681f44d9c400f2e3c32e9e8.png)

## Advanced （高级）
此页面显示一些系统资源信息，USB 设备，文件系统列表，CSS 和固件升级。

![](../../images/posts/6340421b0a2242f2e3a22e6042cdd9ba.png)

## PineAP（菠萝 AP 套件）
PineAP 是一种有效的，模块化恶意接入点套件，旨在帮助用户来连接客户端，通过模仿客户端的首选网络来实现。

![](../../images/posts/0c64acc95e8c939fa4a71d8b56c719b0.png)

### Recon(侦察模式)
用于探测附加的wifi 名称，信道等跟 airodump-ng的wifi探测一样。使用前需要开启PineAP，这会导致Wlan接口占用。

![](../../images/posts/9cd4f691a1bce2c57dfcf4969acdd618.png)
...
![](../../images/posts/ec4be6b0b20ecaf42cbbb2a27973f430.png)

### Tracking（跟踪）
tracking 会持续的扫描指定的客户端设备并执行一个定制化的跟踪脚本。这个特性需要打开 PineAP里的 Log Probes 和 Log Associations。

![](../../images/posts/7bc55304ca0862bf3a98b760752867e4.png)

# 0x07 常用模块
## ConnectedClients【客户管理】
显示当前连接的客户端设备的信息，提供DHCP服务。使用这个就不用开启PineAP了，并且有直接的黑名单管理。使用这个模块可以很直观的看到连接用户的IP和MAC地址。

![](../../images/posts/e6f10bded0a9caf20162062ea8dd0f42.png)

## Dwall 【流量监听】
绵羊墙，可以显示 HTTP URLs, Cookies, 可实时展示客户端的图片。注意只是HTTP。操作十分简单，<font style="color:rgb(51, 51, 51);">Enable开启模块，Start Listening 开始监听。</font>

实际上，有点鸡肋，其实Cookies的获取是比较重要的，但是无法判断Cookie是哪个站点的...如果你熟悉PHP语法，你可以自定义修改一些内容。

![](../../images/posts/771aad800e054fc3a523b2cf9c1b1fbb.png)

## <font style="color:rgb(51, 51, 51);">SSLsplit 【流量抓取】</font>
...
![](../../images/posts/5c4e846f57078670865b3259bcb32d16.png)
...
![](../../images/posts/a1e07d6563752f6fc82a28af5bd2b238.png)
...
![](../../images/posts/76b77a318fab797fa6a487c40ab680d5.png)
...
![](../../images/posts/d286e2f8da48ea7783ea290b945efaea.png)
...
![](../../images/posts/ec177640bab38fbde4cfd1d00a4af265.png)
...
![](../../images/posts/8cad7ddc1c0ff5ccfd1cda38cc0d1542.png)

 下载并将你想要的模板复制到`/root/portals`目录下面去。

![](../../images/posts/ef41217bee1796b7dc412bfc994c2052.png)

你启动服务后，可以在预览中看到模板的加载情况。

![](../../images/posts/1cd325efd0dc1a5fe0029bb6edcf51bf.png)
...
![](../../images/posts/ccfc811671382521f399572d5cd29c9d.png)

2. **删除前端跳转**

`vi /root/portals/mcdonalds-login/index.php`

![](../../images/posts/93c2522340a5d04c4145efa0b1a2d6b6.png)

![](../../images/posts/7fc8399181292b78c2387b380c933d25.png)
...
![](../../images/posts/6709a0f18abb71b55a1233f64d8d0f73.png)
...
![](../../images/posts/ff263802247b1dda2944ee1dc6c865c7.png)
...
![](../../images/posts/ddcf7381f8fb7b4fc78d70c3b395d810.png)

本质上就是调用命令`meterpreter`

![](../../images/posts/c5aa53ffa70813905b1274edd4dfb3f5.png)

然而`multi/handler`有很多中payload，我们先通过`cat /proc/cpuinfo`来获取框架信息，可以看到就是`mips`架构的。

![](../../images/posts/5e05ed62bc8a32e178eef91ab8b40a41.png)

后续我尝试了很多种payload但都无法连接上，不确定是什么原因。

![](../../images/posts/91e0989d2148a60ae27922a52e528879.png)

直到我去官方论坛看到这样的说法。

![](../../images/posts/880848a7cf3928086c2fd5a8b53e05cf.png)
...
![](../../images/posts/76d782db161674a2c254e2deb9df4431.png)

这个文件有1.44MB，然而Shell的执行文件只有356字节，连1kb都没有。

![](../../images/posts/de858f9a8c83dc489278867ec9c8fd8c.png)

上传到`/tmp`目录下，这个目录存储空间大。`<font style="color:rgb(52, 53, 65);">nohup ./ruun5 >/dev/null 2>&1 &</font>`这个命令可以保证在后台运行且不会输出日志（报错日志也不会有），也可以直接使用`<font style="color:rgb(52, 53, 65);">nohup ./ruun5 &</font>`<font style="color:rgb(52, 53, 65);">。</font>

![](../../images/posts/d8c0c9c53c6e18abd84a43178bd9ea0b.png)
...
![](../../images/posts/01c142e39b3f1ac13ce5d62138e3f8a7.png)

执行之后就可以收到`session`了。

![](../../images/posts/3cf9ea0158706ef7e07f533e66060589.png)
...
![](../../images/posts/33b9f232d334e480a0435e720aebc998.png)
...
![](../../images/posts/f12425afa021a5fa7b79aa0c88230c17.png)
...
![](../../images/posts/b7fcd69a26fda3fb557e4027311804f5.png)
...
![](../../images/posts/19b51d3345dba3fe5ed0ab02e152ae5d.png)
...
![](../../images/posts/7b06d503f69dc772f0fd3d24c464fba3.png)

注意**需要把支持隐形代理打开**

![](../../images/posts/c3663bec86dd5021c205d29299109233.png)

开启后即可抓到流量包。

![](../../images/posts/07fa5477777111e69abca975f3fe1171.png)
