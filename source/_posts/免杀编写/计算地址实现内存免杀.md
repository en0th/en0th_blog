---
title: 计算地址实现内存免杀
date: '2024-04-30 10:21:03'
updated: '2026-03-07 20:26:22'
---
<!--more--> 
# 0x00 前言
...
![](../../images/posts/cddcd1b548d2d4d6e473007086031e55.png)![](../../images/posts/f76a28dbe78ae1ee5ec0d1fe08b0294f.png)
...
### 内存解密恶意代码
简单来说：  
1.先置空地址，即"\xe8\x00\x00\x00\x00"+真正的加密后的shellcode

2.编译后通过x64dbg，看到因置空地址自动是下一行的地址。  
3.找到decrypt函数开头压栈地址，与上一步拿到地址相减，得到偏移量。

4.将偏移量按照小端存储的方式，替换掉我们一开始设置的"\x00\x00\x00\x00"即可。
...
```plain
unsigned char shellcode[] = "\xe8\x00\x00\x00\x00......<真正的加密后的shellcode>"
```
...
![](../../images/posts/8be04b429ad66e60ba6f648a78fe8e49.png)

`e8 call`的格式是`CALL Jz`，那么这里的`f64`和`Jz`是什么意思呢，继续翻官方文档：

![](../../images/posts/9d054d0cb522eefdd6e8e87b2202cd72.png)

![](../../images/posts/81f58b2172f6570016f296324b9583a0.png)

![](../../images/posts/270a244e5a83babefb32fbc3b2d06081.png)
...
![](../../images/posts/0270fbda3e3940b9a15f59490e1fc107.png)

![](../../images/posts/162262b93e3278ce119f3bed65368c21.png)

![](../../images/posts/178c816e2290ec3dd91c2b763e3e4a4e.png)

最后将算出的四字节偏移填充到`e8`后面就好了，比如我这里按照Windows的小端存储方式，最后的结果就应该是`e8 2b 10 06 00`：

![](../../images/posts/8ffb8bf66c7bfb6a9800683a8bedc7c9.png)
...
![](../../images/posts/ba9ae91d16f6539ff2bba29d8463b940.png)
...
![](../../images/posts/4cc8c1b08c69c8a456e733f0dc1ee17f.png)
